...
struct AStarNode {
    ...
    AStarNode(): location(-1), g(-1), h(-1), parent(nullptr), timestep(-1) {}
...
private:
    ...
    bool satisfies_constraints(const AStarNode* node, int current_location, int agent_id, const list<Constraint>& constraints);
    bool no_future_constraints(const AStarNode* node, int agent_id, const list<Constraint>& constraints);
